...
docker run \
  --rm \
  --mount 'type=volume,source=ndc-sqlserver-component-benchmarks_benchmark_output,target=/output' \
  busybox \
  tar c -C /output . \
  | tar x -C "$directory"
...
docker run \
  --rm \
  --mount 'type=volume,source=sqlserver-ndc-component-benchmarks_metrics_output,target=/output' \
  busybox \
  tar c -C /output . \
  | tar x -C "$directory"

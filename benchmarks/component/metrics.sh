...
REQUEST_TIME=$(cat $directory/metrics.txt \
  | sed -n -e '/^ndc_sqlserver_query_total_time_sum/p' \
  | cut -d' ' -f2 )

CONNECTION_ACQUISITION_TIME=$(cat $directory/metrics.txt \
  | sed -n -e '/^ndc_sqlserver_connection_acquisition_wait_time_sum/p' \
  | cut -d' ' -f2 )

QUERY_TIME=$(cat $directory/metrics.txt \
  | sed -n -e '/^ndc_sqlserver_query_execution_time_sum/p' \
  | cut -d' ' -f2 )

REQUESTS_COUNT=$(cat $directory/metrics.txt \
  | sed -n -e '/^ndc_sqlserver_query_total_time_count/p' \
  | cut -d' ' -f2 )
...
tar c -C "$directory" . \
  | docker run \
      --rm \
      --interactive \
      --mount 'type=volume,source=sqlserver-ndc-component-benchmarks_metrics_output,target=/input' \
      busybox \
      tar x -C /input .

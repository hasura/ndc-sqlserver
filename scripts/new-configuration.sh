...
if [[ $# -lt 2 ]]; then
  echo >&2 "Usage: $0 CONFIGURATION-SERVER SQLSERVER-CONNECTION-URI [EXTRA-CONFIG]"
  echo >&2
...
get "http://${CONFIGURATION_SERVER}/" \
  | jq --arg uri "$SQLSERVER_CONNECTION_URI" --argjson extra "$EXTRA_CONFIG" '. * $extra + {"mssql_connection_string": $uri}' \

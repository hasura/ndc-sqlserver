...
PRESERVED_DATA="$(jq '{"mssql_connection_string": .mssql_connection_string}' "$CHINOOK_DEPLOYMENT")"

# Native queries should inform the initial configuration call
INITIAL_DATA="$(jq '{"metadata": {"nativeQueries": .metadata.nativeQueries}}' "$CHINOOK_DEPLOYMENT")"

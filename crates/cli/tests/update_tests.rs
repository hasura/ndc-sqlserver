...
const CONNECTION_URI: &str = "Server=localhost,64003;Uid=SA;Database=Chinook;Pwd=Password!;TrustServerCertificate=true";

#[tokio::test]
async fn test_update_configuration() -> anyhow::Result<()> {
    let dir = tempfile::tempdir()?;

    let connection_uri = configuration::ConnectionUri(configuration::secret::Secret::FromEnvironment {
...

    {
        ...
        fs::write(configuration_file_path.clone(), serde_json::to_string(&input)?).await?;
